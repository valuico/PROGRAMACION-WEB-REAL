...
export function buildProductsFromRows(rows) {
  ...
  rows.forEach((row) => {
    const normalized = {
      ...
      img: row.imagen_url || '/foundation-haze.png',
      tones: Array.isArray(row.tonos) ? row.tonos : [],
      categoria: row.categoria,
      isNew: Boolean(row.es_nuevo),
    };

    if (row.tipo === 'skincare') {
      grouped.skincare.push(normalized);
    } else {
      grouped.makeup.push(normalized);
    }
  });
...
export function normalizeCartItems(cartRows) {
  return cartRows.map((row) => ({
    ...
    img: row.producto?.imagen_url || '/foundation-haze.png',
